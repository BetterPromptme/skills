...
- `skillmd/build-skillmd.ts` — Core rendering logic. Exports `buildSkillmd(params)` which compiles `template/SKILL.md` via Handlebars with computed template variables (text inputs table, image inputs table, example command, etc.).
...
- `SKILL.md.d.ts` — TypeScript module declaration allowing `.md` imports as string.
- `package.json` — Bun project config. Dependencies: `handlebars`.

## Key patterns

- Imports `template/SKILL.md` from repo root using `with { type: "text" }` (Bun text import).
