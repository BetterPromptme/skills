...
Use the frontmatter's `name` as the positional argument (for this skill, use `productivity-coach-and-reduced-procrastination`).

Command form:

```bash
betterprompt generate productivity-coach-and-reduced-procrastination \
  [--input <key>=<value>] \
  [--model <model>] \
