---
name: minimal-studio-denim-portrait
description: This prompt instructs an image-generation model to create a photorealistic vertical studio fashion portrait that uses the provided photo as the definitive reference for the subject’s exact identity and ethnicity, while changing the scene to a clean minimalist studio setting. It specifies wardrobe, barefoot styling, natural makeup, a relaxed seated floor pose, lens/composition, soft diffused daylight, and a neutral editorial color grade, with extensive constraints to prevent any identity drift, facial/feature alterations, anatomical errors, artifacts, stylization, or distracting background elements, resulting in a high-resolution modern lookbook-style image that still clearly depicts the same person as the input.
...
Use the frontmatter's `name` as the positional argument (for this skill, use `minimal-studio-denim-portrait`).

Command form:

```bash
betterprompt generate minimal-studio-denim-portrait \
  [--image-input-url <url>] \
