---
name: girl-on-screen-at-concert
description: 
...
Use the frontmatter's `name` as the positional argument (for this skill, use `girl-on-screen-at-concert`).

Command form:

```bash
betterprompt generate girl-on-screen-at-concert \
  [--image-input-url <url>] \
