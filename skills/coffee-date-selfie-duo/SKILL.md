...
| ---: | ---: | --- | --- |
| 1 | Yes | Upload a clear photo of the first woman | `--image-input-url <url>` or `--image-input-path <path>` |
| 2 | Yes | Upload a clear photo of the second woman | `--image-input-url <url>` or `--image-input-path <path>` |
...
- Required images:
  - **Exactly 2** images: image 1 (Upload a clear photo of the first woman) and image 2 (Upload a clear photo of the second woman). Images must be provided in this order.
...
If the required images are missing, **ask the human for what's missing**. Do not assume or fabricate values. Tell the human: **"Please provide images in this order: image 1 (Upload a clear photo of the first woman) and image 2 (Upload a clear photo of the second woman)"**.

### Step 2: Run via BetterPrompt CLI

Use the frontmatter's `name` as the positional argument (for this skill, use `coffee-date-selfie-duo`).

Command form:

```bash
betterprompt generate coffee-date-selfie-duo \
  [--image-input-url <url>] \
