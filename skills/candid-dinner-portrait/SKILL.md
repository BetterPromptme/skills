---
name: candid-dinner-portrait
description: This prompt instructs an image model/editor to produce a vertical, medium-framed luxury dinner portrait in a modern upscale restaurant, capturing an intimate, softly cinematic nightlife editorial look with warm ambient lighting, shallow depth of field, and detailed table props and materials. It tightly constrains the subject’s identity and natural facial/anatomical features while specifying pose, styling (hair, makeup, outfit), environment, color palette, and realistic texture/lighting behavior, resulting in a polished yet candid magazine-quality fine-dining scene with a quiet-luxury mood.
...
This prompt instructs an image model/editor to produce a vertical, medium-framed luxury dinner portrait in a modern upscale restaurant, capturing an intimate, softly cinematic nightlife editorial look with warm ambient lighting, shallow depth of field, and detailed table props and materials. It tightly constrains the subject’s identity and natural facial/anatomical features while specifying pose, styling (hair, makeup, outfit), environment, color palette, and realistic texture/lighting behavior, resulting in a polished yet candid magazine-quality fine-dining scene with a quiet-luxury mood.
...
Use the frontmatter's `name` as the positional argument (for this skill, use `candid-dinner-portrait`).

Command form:

```bash
betterprompt generate candid-dinner-portrait \
  [--image-input-url <url>] \
