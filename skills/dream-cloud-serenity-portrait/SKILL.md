---
name: dream-cloud-serenity-portrait
description: This prompt instructs an image-generation/editing model to create a high-resolution, ultra-realistic vertical (9:16) surreal fashion photograph that keeps the uploaded person’s facial identity and key traits unchanged, placing them reclining on a soft, voluminous cloud in a clean gradient blue sky with bright diffused daylight and an airy pastel editorial finish. It specifies composition, camera perspective, pose constraints, outfit styling, lighting, and color palette while listing strict negatives to prevent identity drift, unrealistic rendering, artifacts, cluttered backgrounds, or incorrect posing, resulting in a recognizable, dreamlike sky portrait centered on the same subject.
...
Use the frontmatter's `name` as the positional argument (for this skill, use `dream-cloud-serenity-portrait`).

Command form:

```bash
betterprompt generate dream-cloud-serenity-portrait \
  [--image-input-url <url>] \
