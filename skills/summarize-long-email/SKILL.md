...
| --- | ---: | --- | --- |
| `Email_thread` | Required | Paste the full email conversation here | (none) |
...
Use the frontmatter's `name` as the positional argument (for this skill, use `summarize-long-email`).

Command form:

```bash
betterprompt generate summarize-long-email \
  [--input <key>=<value>] \
  [--model <model>] \
