...
Use the frontmatter's `name` as the positional argument (for this skill, use `female-only-retro-flash-party-portrait`).

Command form:

```bash
betterprompt generate female-only-retro-flash-party-portrait \
  [--image-input-url <url>] \
