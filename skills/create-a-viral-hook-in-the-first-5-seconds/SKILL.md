...
Use the frontmatter's `name` as the positional argument (for this skill, use `create-a-viral-hook-in-the-first-5-seconds`).

Command form:

```bash
betterprompt generate create-a-viral-hook-in-the-first-5-seconds \
  [--input <key>=<value>] \
  [--model <model>] \
