...
| ---: | ---: | --- | --- |
| 1 | Yes | Photo | `--image-input-url <url>` or `--image-input-path <path>` |
...
Use the frontmatter's `name` as the positional argument (for this skill, use `rain-drops`).

Command form:

```bash
betterprompt generate rain-drops \
  [--image-input-url <url>] \
