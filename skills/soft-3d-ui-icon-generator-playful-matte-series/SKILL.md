...
Use the frontmatter's `name` as the positional argument (for this skill, use `soft-3d-ui-icon-generator-playful-matte-series`).

Command form:

```bash
betterprompt generate soft-3d-ui-icon-generator-playful-matte-series \
  [--input <key>=<value>] \
  [--model <model>] \
