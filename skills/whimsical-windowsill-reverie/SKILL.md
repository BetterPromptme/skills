...
Use the frontmatter's `name` as the positional argument (for this skill, use `whimsical-windowsill-reverie`).

Command form:

```bash
betterprompt generate whimsical-windowsill-reverie \
  [--input <key>=<value>] \
  [--model <model>] \
