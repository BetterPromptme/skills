...
Use the frontmatter's `name` as the positional argument (for this skill, use `cozy-christmas-portrait-with-cat`).

Command form:

```bash
betterprompt generate cozy-christmas-portrait-with-cat \
  [--image-input-url <url>] \
