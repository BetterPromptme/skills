---
name: youth-in-the-night-lights
description: 
...
Use the frontmatter's `name` as the positional argument (for this skill, use `youth-in-the-night-lights`).

Command form:

```bash
betterprompt generate youth-in-the-night-lights \
  [--image-input-url <url>] \
