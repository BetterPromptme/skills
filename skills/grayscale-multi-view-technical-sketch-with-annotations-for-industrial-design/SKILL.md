...
- Optional: model and options.
  - Present the human with the default model **`gpt-image-1`** and its available options. Look up `gpt-image-1` in the `betterprompt resources` output (filtered to modality `"image"`) and show its `availableOptions` as: `key: val1, val2 (default), val3  |  key2: ...`. Mark a value `(default)` if it matches these defaults: `{"size":"1536x1024","image":{"n":0,"size":1,"quality":1},"quality":"medium"}`.
  - If the human does not specify, defaults are used: model `gpt-image-1`, options `{"size":"1536x1024","image":{"n":0,"size":1,"quality":1},"quality":"medium"}`. Other models from the resources call are also available.
...
Use the frontmatter's `name` as the positional argument (for this skill, use `grayscale-multi-view-technical-sketch-with-annotations-for-industrial-design`).

Command form:

```bash
betterprompt generate grayscale-multi-view-technical-sketch-with-annotations-for-industrial-design \
  [--image-input-url <url>] \
...
- If the human does **not** mention options, **omit** `--options` and BetterPrompt will use the default options: **`{"size":"1536x1024","image":{"n":0,"size":1,"quality":1},"quality":"medium"}`**.
...
betterprompt generate grayscale-multi-view-technical-sketch-with-annotations-for-industrial-design \
  ...
  --model gpt-image-1 \
  --options '{"size":"1536x1024","image":{"n":0,"size":1,"quality":1},"quality":"medium"}'
```
