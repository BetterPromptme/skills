...
| --- | ---: | --- | --- |
| `your_name` | Optional |  | `Potter` |
| `your_house` | Required | Allowed options: ravenclaw, slytherin, gryffindor, hufflepuff | (none) |
| `house_color` | Required | The color of your house Allowed options: red_with_gold_accent, blue_with_bronze_accent, green_with_silver_accent, yellow_with_black_accent | (none) |
| `shirt_number` | Optional | Number you want on your shirt | `7` |
...
- Optional text inputs (use defaults if not provided by the human):
    - `your_name` (default: `Potter`)
  - `shirt_number` (default: `7`)
- Required images:
...
Use the frontmatter's `name` as the positional argument (for this skill, use `quitditch-photo-maker`).

Command form:

```bash
betterprompt generate quitditch-photo-maker \
  [--input <key>=<value>] \
  [--image-input-url <url>] \
...
```bash
betterprompt generate quitditch-photo-maker \
  --input your_name=Potter \
  --input your_house=ravenclaw \
  --input house_color=red_with_gold_accent \
  --input shirt_number=7 \
