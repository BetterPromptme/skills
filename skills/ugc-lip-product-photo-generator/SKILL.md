...
| 2 | Yes | Image 2: Lip product photo | `--image-input-url <url>` or `--image-input-path <path>` |
...
Use the frontmatter's `name` as the positional argument (for this skill, use `ugc-lip-product-photo-generator`).

Command form:

```bash
betterprompt generate ugc-lip-product-photo-generator \
  [--image-input-url <url>] \
