...
Use the frontmatter's `name` as the positional argument (for this skill, use `holographic-self-mirror-absolute-face-lock`).

Command form:

```bash
betterprompt generate holographic-self-mirror-absolute-face-lock \
  [--image-input-url <url>] \
