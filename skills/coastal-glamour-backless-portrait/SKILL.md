---
name: coastal-glamour-backless-portrait
description: This prompt instructs an AI image generator to produce an ultra-photorealistic, high-end fashion editorial portrait of a woman posed in an elegant over-the-shoulder stance beside a tranquil tropical shoreline, with specific direction for wardrobe (a blush satin open-back gown), jewelry, hair and makeup, lighting, color palette, lens choice, and background elements to achieve a premium resort-campaign aesthetic. The expected result is a magazine-quality coastal luxury image with cinematic depth of field, realistic skin and fabric detail, refined mood, and explicit exclusions to avoid common visual artifacts or unwanted styles.
...
Use the frontmatter's `name` as the positional argument (for this skill, use `coastal-glamour-backless-portrait`).

Command form:

```bash
betterprompt generate coastal-glamour-backless-portrait \
  [--image-input-url <url>] \
