...
| --- | ---: | --- | --- |
| `Tasks` | Required | Paste your list of tasks here | (none) |
| `Context` | Required | e.g., deadlines, available hours, energy level, role | (none) |
...
Use the frontmatter's `name` as the positional argument (for this skill, use `priority-planner`).

Command form:

```bash
betterprompt generate priority-planner \
  [--input <key>=<value>] \
  [--model <model>] \
