...
Use the frontmatter's `name` as the positional argument (for this skill, use `flexible-ai-assistant-for-everyday-work`).

Command form:

```bash
betterprompt generate flexible-ai-assistant-for-everyday-work \
  [--input <key>=<value>] \
  [--model <model>] \
