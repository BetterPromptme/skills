...
| ---: | ---: | --- | --- |
| 1 | Yes | A clear headshot of yourself in natural light (preferably without make-up) | `--image-input-url <url>` or `--image-input-path <path>` |
...
- Required images:
  - **Exactly 1** images: image 1 (A clear headshot of yourself in natural light (preferably without make-up)). Images must be provided in this order.
...
If the required images are missing, **ask the human for what's missing**. Do not assume or fabricate values. Tell the human: **"Please provide images in this order: image 1 (A clear headshot of yourself in natural light (preferably without make-up))"**.

### Step 2: Run via BetterPrompt CLI

Use the frontmatter's `name` as the positional argument (for this skill, use `personal-color-generator`).

Command form:

```bash
betterprompt generate personal-color-generator \
  [--image-input-url <url>] \
