---
name: golden-hour-wildflower-portrait
description: This prompt instructs an image model to generate a photorealistic, high-end editorial vertical (9:16) golden-hour portrait set in a lush cosmos-filled wildflower garden, using the uploaded image as the sole identity reference so the subject’s facial features, skin tone, hair, and eye color remain exactly recognizable. It specifies wardrobe, makeup, pose, camera/lens settings, depth-of-field, lighting, color grading, and background composition to create a dreamy romantic floral tunnel look, while explicitly banning identity changes, occlusions, artifacts, stylization, and common quality issues to ensure a sharp, natural, ultra-detailed final image.
...
Use the frontmatter's `name` as the positional argument (for this skill, use `golden-hour-wildflower-portrait`).

Command form:

```bash
betterprompt generate golden-hour-wildflower-portrait \
  [--image-input-url <url>] \
