...
Use the frontmatter's `name` as the positional argument (for this skill, use `formed-by-feathers-minimalist-shape`).

Command form:

```bash
betterprompt generate formed-by-feathers-minimalist-shape \
  [--input <key>=<value>] \
  [--model <model>] \
