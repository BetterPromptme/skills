...
Use the frontmatter's `name` as the positional argument (for this skill, use `multi-constraint-ladder-synthesiser`).

Command form:

```bash
betterprompt generate multi-constraint-ladder-synthesiser \
  [--input <key>=<value>] \
  [--model <model>] \
