---
name: visualize-brighter-moments
description: 
...
| --- | ---: | --- | --- |
| `drink_and_container` | Optional |  | `a transparent wine glass filled with red wine` |
...
| ---: | ---: | --- | --- |
| 1 | Yes | Ypur photo | `--image-input-url <url>` or `--image-input-path <path>` |
...
- Optional text inputs (use defaults if not provided by the human):
    - `drink_and_container` (default: `a transparent wine glass filled with red wine`)
- Required images:
  - **Exactly 1** images: image 1 (Ypur photo). Images must be provided in this order.
...
If the required images are missing, **ask the human for what's missing**. Do not assume or fabricate values. Tell the human: **"Please provide images in this order: image 1 (Ypur photo)"**.

### Step 2: Run via BetterPrompt CLI

Use the frontmatter's `name` as the positional argument (for this skill, use `visualize-brighter-moments`).

Command form:

```bash
betterprompt generate visualize-brighter-moments \
  [--input <key>=<value>] \
  [--image-input-url <url>] \
...
```bash
betterprompt generate visualize-brighter-moments \
  --input 'drink_and_container=a transparent wine glass filled with red wine' \
