...
| --- | ---: | --- | --- |
| `product` | Optional | The product or service you want to create a customer persona for | `Project management tool for remote creative agencies` |
| `industry` | Optional | The industry or market sector your product serves | `Creative services` |
...
- Optional text inputs (use defaults if not provided by the human):
    - `product` (default: `Project management tool for remote creative agencies`)
  - `industry` (default: `Creative services`)
...
Use the frontmatter's `name` as the positional argument (for this skill, use `customer-persona-builder`).

Command form:

```bash
betterprompt generate customer-persona-builder \
  [--input <key>=<value>] \
  [--model <model>] \
...
```bash
betterprompt generate customer-persona-builder \
  --input 'product=Project management tool for remote creative agencies' \
  --input 'industry=Creative services' \
  --model gpt-5.2 \
