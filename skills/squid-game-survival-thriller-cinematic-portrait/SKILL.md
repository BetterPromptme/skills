...
Use the frontmatter's `name` as the positional argument (for this skill, use `squid-game-survival-thriller-cinematic-portrait`).

Command form:

```bash
betterprompt generate squid-game-survival-thriller-cinematic-portrait \
  [--image-input-url <url>] \
