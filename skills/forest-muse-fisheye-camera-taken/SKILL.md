...
Use the frontmatter's `name` as the positional argument (for this skill, use `forest-muse-fisheye-camera-taken`).

Command form:

```bash
betterprompt generate forest-muse-fisheye-camera-taken \
  [--image-input-url <url>] \
