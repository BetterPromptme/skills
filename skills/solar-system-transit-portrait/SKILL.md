---
name: solar-system-transit-portrait
description: This prompt instructs an image generator to create a photorealistic, vertical cinematic sci‑fi portrait of the same person shown in the provided reference image, strictly preserving their facial identity and key features. It places the subject seated inside a sleek orbital transit cabin beside a panoramic window with Earth and external station details visible, specifies camera framing, lens feel, pose, outfit, lighting, reflections, and color grading for a high‑budget movie still look, and includes constraints to prevent identity drift, stylization, artifacts, or low-quality results.
...
Use the frontmatter's `name` as the positional argument (for this skill, use `solar-system-transit-portrait`).

Command form:

```bash
betterprompt generate solar-system-transit-portrait \
  [--image-input-url <url>] \
