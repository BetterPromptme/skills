...
| --- | ---: | --- | --- |
| `style` | Required | The primary style for the logo concepts. Allowed options: icon, emblem, abstract, wordmark, lettermark | (none) |
| `industry` | Optional | The industry the brand operates in (e.g., tech, fashion, food). | `Productivity SaaS` |
| `brandName` | Optional | The name of the brand or company. | `Peakflow` |
...
- Optional text inputs (use defaults if not provided by the human):
    - `industry` (default: `Productivity SaaS`)
  - `brandName` (default: `Peakflow`)
...
Use the frontmatter's `name` as the positional argument (for this skill, use `brand-logo-concept`).

Command form:

```bash
betterprompt generate brand-logo-concept \
  [--input <key>=<value>] \
  [--model <model>] \
...
betterprompt generate brand-logo-concept \
  --input style=icon \
  --input 'industry=Productivity SaaS' \
  --input brandName=Peakflow \
