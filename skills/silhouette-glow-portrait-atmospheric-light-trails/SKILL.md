...
Use the frontmatter's `name` as the positional argument (for this skill, use `silhouette-glow-portrait-atmospheric-light-trails`).

Command form:

```bash
betterprompt generate silhouette-glow-portrait-atmospheric-light-trails \
  [--input <key>=<value>] \
  [--model <model>] \
