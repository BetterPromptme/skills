...
Use the frontmatter's `name` as the positional argument (for this skill, use `landmark-to-furniture-photorealistic-concept`).

Command form:

```bash
betterprompt generate landmark-to-furniture-photorealistic-concept \
  [--input <key>=<value>] \
  [--model <model>] \
