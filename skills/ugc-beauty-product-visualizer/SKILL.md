...
skillVersionId: 019b07c8-aab0-74ef-8e56-ae445e55ae29
---

## What it does

This prompt instructs an AI to turn a user’s standard portrait into a clean, studio-style UGC beauty image where the same person is shown in a shoulders-up, commercially appropriate framing while naturally holding the exact product from a separate uploaded product photo. The result is a premium, minimal campaign-like visual with soft diffused lighting, a pastel background matched to the product’s main color, preserved identity and realistic skin texture, and strict brand-accurate product rendering (logo, label, colors, packaging) without redesigns, distortions, or sexualized styling.
...
| ---: | ---: | --- | --- |
| 1 | Yes | Upload a clear photo of the exact product you want featured. | `--image-input-url <url>` or `--image-input-base64 <base64>` or `--image-input-path <absolute path to image>` |
| 2 | Yes | Upload your normal portrait photo you want to convert into a UGC image. | `--image-input-url <url>` or `--image-input-base64 <base64>` or `--image-input-path <absolute path to image>` |
...
- Required images:
  - **Exactly 2** images: image 1 (Upload a clear photo of the exact product you want featured.) and image 2 (Upload your normal portrait photo you want to convert into a UGC image.). Images must be provided in this order.
...
If the required images are missing, **ask the human for what's missing**. Do not assume or fabricate values. Tell the human: **"Please provide images in this order: image 1 (Upload a clear photo of the exact product you want featured.) and image 2 (Upload your normal portrait photo you want to convert into a UGC image.)"**.

### Step 2: Run via BetterPrompt CLI

Use the frontmatter's `skillVersionId` as the positional argument (for this skill version, use `019b07c8-aab0-74ef-8e56-ae445e55ae29`).

Command form:

```bash
betterprompt generate 019b07c8-aab0-74ef-8e56-ae445e55ae29 \
  [--image-input-url <url>] \
