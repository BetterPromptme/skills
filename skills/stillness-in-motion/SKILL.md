...
description: This prompt directs an image generation/edit to produce a polished luxury fashion editorial scene by a modern minimalist pool, featuring the original subject unchanged in identity, outfit, and physical traits while posing calmly on a deep green inflatable float. It specifies sport-inspired styling, clean architecture, natural directional sunlight, and balanced negative-space composition to achieve a serene, controlled, high-end finish with natural skin texture and no added text, logos, or watermarks.
...
Use the frontmatter's `name` as the positional argument (for this skill, use `stillness-in-motion`).

Command form:

```bash
betterprompt generate stillness-in-motion \
  [--image-input-url <url>] \
