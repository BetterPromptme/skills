...
Use the frontmatter's `name` as the positional argument (for this skill, use `conflict-resolution-strategist`).

Command form:

```bash
betterprompt generate conflict-resolution-strategist \
  [--input <key>=<value>] \
  [--model <model>] \
