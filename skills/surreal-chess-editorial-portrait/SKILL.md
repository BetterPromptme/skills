---
name: surreal-chess-editorial-portrait
description: This prompt instructs an image-generation model to create an ultra-realistic, vertical high-fashion editorial portrait that closely matches the uploaded person’s facial identity and hair while placing them in a dark, cinematic studio with a black-and-cream checkerboard set and oversized glossy chess pieces in a specified accent color. It defines the wardrobe, pose, camera viewpoint, lighting, and photorealistic finishing details to achieve a surreal chess-themed scene, and includes constraints to prevent identity drift, incorrect colors, anatomical errors, or non-photographic styles, resulting in a sharp, recognizable, high-resolution portrait of the same subject surrounded by dramatic chess elements.
...
| --- | ---: | --- | --- |
| `chess_piece_color_` | Required |  | (none) |
...
Use the frontmatter's `name` as the positional argument (for this skill, use `surreal-chess-editorial-portrait`).

Command form:

```bash
betterprompt generate surreal-chess-editorial-portrait \
  [--input <key>=<value>] \
  [--image-input-url <url>] \
...
```bash
betterprompt generate surreal-chess-editorial-portrait \
  --input chess_piece_color_=<value> \
