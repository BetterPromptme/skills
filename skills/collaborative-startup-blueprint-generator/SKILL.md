...
Use the frontmatter's `name` as the positional argument (for this skill, use `collaborative-startup-blueprint-generator`).

Command form:

```bash
betterprompt generate collaborative-startup-blueprint-generator \
  [--input <key>=<value>] \
  [--model <model>] \
