...
Use the frontmatter's `name` as the positional argument (for this skill, use `luxury-lifestyle-identity-preserving-portrait`).

Command form:

```bash
betterprompt generate luxury-lifestyle-identity-preserving-portrait \
  [--image-input-url <url>] \
