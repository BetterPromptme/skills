---
name: minimalist-urban-streetside-selfie
description: This prompt instructs an AI image generator to produce an ultra-photorealistic, handheld smartphone-style selfie of a young woman in a clean-girl, luxury-casual look standing centered outside a modern high-end building entrance in bright daytime. It specifies precise details for pose, expression, camera angle and distortion, facial features, makeup, hair, accessories, outfit textures, architecture, pavement elements, lighting, reflections, color palette, and overall editorial-social realism, while also listing exclusions to avoid artificial, stylized, or artifact-prone results. The intended outcome is a social-media-ready yet premium fashion-editorial portrait that feels authentically captured in an upscale urban setting.
...
Use the frontmatter's `name` as the positional argument (for this skill, use `minimalist-urban-streetside-selfie`).

Command form:

```bash
betterprompt generate minimalist-urban-streetside-selfie \
  [--image-input-url <url>] \
