---
name: hollywood-subway-grate-portrait
description: This prompt instructs an image generator to produce an ultra‑realistic, vertical, full‑body classic Hollywood street fashion portrait that keeps the uploaded person’s facial identity exactly the same while changing only the hairstyle to a Marilyn Monroe–inspired platinum curled bob, placing the subject on a cinematic New York street over a subway grate in a wind‑blown white halter pleated dress pose. The intended outcome is a sharp, high-end, vintage editorial photograph with controlled lighting, shallow depth of field, accurate anatomy and fabric motion, and explicit constraints to avoid identity drift, artifacts, stylization, or low-quality rendering.
...
Use the frontmatter's `name` as the positional argument (for this skill, use `hollywood-subway-grate-portrait`).

Command form:

```bash
betterprompt generate hollywood-subway-grate-portrait \
  [--image-input-url <url>] \
