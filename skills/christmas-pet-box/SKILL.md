...
skillVersionId: 019b11e6-14b8-73ec-a60d-261559d9c167
---

## What it does

This prompt instructs the AI to take a user-provided pet photo and generate a single, high-resolution, photorealistic Christmas studio image that preserves the pet’s exact identity while placing it centered inside a warm, golden-brown cardboard box setup with soft festive lighting, fairy lights, snowy mini trees, and a cute holiday outfit, resulting in a clean, cozy, professionally staged holiday photoshoot look.
...
| ---: | ---: | --- | --- |
| 1 | Yes | Upload your pet's photo | `--image-input-url <url>` or `--image-input-base64 <base64>` or `--image-input-path <absolute path to image>` |
...
- Required images:
  - **Exactly 1** images: image 1 (Upload your pet's photo). Images must be provided in this order.
...
If the required images are missing, **ask the human for what's missing**. Do not assume or fabricate values. Tell the human: **"Please provide images in this order: image 1 (Upload your pet's photo)"**.

### Step 2: Run via BetterPrompt CLI

Use the frontmatter's `skillVersionId` as the positional argument (for this skill version, use `019b11e6-14b8-73ec-a60d-261559d9c167`).

Command form:

```bash
betterprompt generate 019b11e6-14b8-73ec-a60d-261559d9c167 \
  [--image-input-url <url>] \
