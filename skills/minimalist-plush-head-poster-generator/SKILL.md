...
Use the frontmatter's `name` as the positional argument (for this skill, use `minimalist-plush-head-poster-generator`).

Command form:

```bash
betterprompt generate minimalist-plush-head-poster-generator \
  [--input <key>=<value>] \
  [--model <model>] \
