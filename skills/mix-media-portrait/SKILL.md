...
| `gaze_direction` | Required |  | (none) |
| `secondary_color` | Required |  | (none) |
| `accessories/styling` | Required |  | (none) |
...
If any required text input or the required images are missing, **ask the human for what's missing**. Do not assume or fabricate values. Tell the human: **"Please provide images in this order: image 1 (your selfie)"**.

### Step 2: Run via BetterPrompt CLI

Use the frontmatter's `name` as the positional argument (for this skill, use `mix-media-portrait`).

Command form:

```bash
betterprompt generate mix-media-portrait \
  [--input <key>=<value>] \
  [--image-input-url <url>] \
...
betterprompt generate mix-media-portrait \
  ...
  --input gaze_direction=<value> \
  --input secondary_color=<value> \
  --input accessories/styling=<value> \
