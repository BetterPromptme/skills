...
| 2 | Yes | Upload Faker's photo | `--image-input-url <url>` or `--image-input-path <path>` |
...
Use the frontmatter's `name` as the positional argument (for this skill, use `faker-duo-photo`).

Command form:

```bash
betterprompt generate faker-duo-photo \
  [--image-input-url <url>] \
